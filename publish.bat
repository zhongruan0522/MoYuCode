...
call dotnet publish "%ROOT%src\OneCode.Win\OneCode.Win.csproj" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:DebugType=None -p:DebugSymbols=false -o "%WIN_TRAY_DIR%"
if errorlevel 1 goto :error
if not exist "%WIN_TRAY_DIR%\OneCode.Win.exe" (
  echo WinForms publish did not produce "OneCode.Win.exe".
  goto :error
)
copy /y "%WIN_TRAY_DIR%\OneCode.Win.exe" "%PUBLISH_DIR%\OneCode.Win.exe" >nul
rmdir /s /q "%WIN_TRAY_DIR%"

echo.
echo Done.
echo Output: "%PUBLISH_DIR%"
echo Tray app: "%PUBLISH_DIR%\OneCode.Win.exe"
