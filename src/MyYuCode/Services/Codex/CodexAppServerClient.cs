...
namespace MyYuCode.Services.Codex;
...
public sealed class CodexAppServerClient : IAsyncDisposable
{
    ...
    private ProcessStartInfo CreateCodexStartInfo()
    {
        ...
        if (!string.IsNullOrWhiteSpace(configuredExecutable))
        {
            ...
            AddMyYuCodeProviderApiKeys(startInfo);
            return startInfo;
        }
    ...
    private void AddMyYuCodeProviderApiKeys(ProcessStartInfo startInfo)
    {
        try
        {
            ...
            foreach (var provider in _store.Providers)
            {
                ...
                var envName = $"MYYUCODE_API_KEY_{provider.Id:N}";
