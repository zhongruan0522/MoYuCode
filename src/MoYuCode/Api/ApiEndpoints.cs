...
using MoYuCode.Contracts.FileSystem;
using MoYuCode.Contracts.Git;
using MoYuCode.Contracts.Jobs;
using MoYuCode.Contracts.Projects;
using MoYuCode.Contracts.Providers;
using MoYuCode.Contracts.Skills;
using MoYuCode.Contracts.Tools;
using MoYuCode.Data;
...
{
    public static void MapMyYuCodeApis(this WebApplication app)
    {
    ...
    private static void MapProjects(RouteGroupBuilder api)
    {
            ...
            TerminalWindowLauncher terminalWindowLauncher) =>
        {
            ...
            terminalWindowLauncher.LaunchCommandWindow(
                ...
                windowTitle: $"MoYuCode（摸鱼Coding） - {project.ToolType} - {project.Name}",
                argv: launch.Args,
    ...
    private static (IReadOnlyList<string> Args, IReadOnlyDictionary<string, string>? Env) BuildCodexLaunch(ProjectEntity project)
    {
        ...
        const string providerId = "myyucode";
        const string envKeyName = "MYYUCODE_API_KEY";
        ...
        if (project.Provider is not null)
        {
            ...
            args.Add("-c");
            args.Add($"model_providers.{providerId}.name=\"MoYuCode（摸鱼Coding）\"");
    ...
    private static void MapSkills(RouteGroupBuilder api)
    {
            ...
            [FromQuery] string targetService) =>
        {
            ...
            {
                throw new ApiHttpException(StatusCodes.Status400BadRequest, "Skill name is required.");
            }
            ...
            var normalizedService = targetService.ToLowerInvariant();
            if (normalizedService != "codex" && normalizedService != "claudecode")
            {
                throw new ApiHttpException(StatusCodes.Status400BadRequest, "Target service must be 'codex' or 'claudeCode'.");
            }

            var userProfile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
            var targetDir = normalizedService switch
            {
                "codex" => Path.Combine(userProfile, ".codex", "skills", ".system", skillName),
                "claudecode" => Path.Combine(userProfile, ".claude", "skills", skillName),
                _ => throw new InvalidOperationException("Invalid target service")
            ...
            if (!Directory.Exists(targetDir))
            {
                throw new ApiHttpException(StatusCodes.Status404NotFound, "Skill not found or not installed.");
            }

            try
            {
                Directory.Delete(targetDir, recursive: true);
                return Results.Ok(new { success = true, message = $"Skill '{skillName}' uninstalled from {targetService}." });
            }
            catch (IOException ex)
            {
                throw new ApiHttpException(StatusCodes.Status500InternalServerError, $"Failed to uninstall skill: {ex.Message}");
            }
            catch (UnauthorizedAccessException)
            {
                throw new ApiHttpException(StatusCodes.Status500InternalServerError, "Permission denied. Unable to delete skill directory.");
            }
        });
    }
}
