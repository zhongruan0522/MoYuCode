...
internal sealed class TrayAppContext : ApplicationContext
{
    ...
    public TrayAppContext()
    {
        ...
        startItem = new ToolStripMenuItem("启动 MyYuCode（摸鱼Coding）", null, async (_, __) => await StartAppAsync());
        stopItem = new ToolStripMenuItem("关闭 MyYuCode（摸鱼Coding）", null, async (_, __) => await StopAppAsync());
        openItem = new ToolStripMenuItem("打开 MyYuCode（摸鱼Coding）", null, (_, __) => OpenUrl());
        ...
        var iconStream = typeof(TrayAppContext).Assembly.GetManifestResourceStream("MyYuCode.Win.favicon.ico");
        trayIcon = new NotifyIcon
        {
            Text = "MyYuCode（摸鱼Coding）",
    ...
    private void RefreshMenu()
    {
        ...
        trayIcon.Text = isRunning
            ? "MyYuCode（摸鱼Coding） (Running)"
            : "MyYuCode（摸鱼Coding） (Stopped)";
    }
    ...
    private async Task StartAppAsync()
    {
        ...
        try
        {
            app ??= MyYuCodeApp.Create(Array.Empty<string>(), out _);
            await app.StartAsync();
            isRunning = true;
        }
        catch (Exception ex)
        {
            MessageBox.Show(ex.Message, "Start MyYuCode（摸鱼Coding） failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
            if (app != null)
            {
    ...
    private async Task StopAppAsync()
    {
        ...
        {
            MessageBox.Show(ex.Message, "Stop MyYuCode（摸鱼Coding） failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        finally
        {
            isStopping = false;
