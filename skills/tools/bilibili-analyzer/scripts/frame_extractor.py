...
class FrameExtractor:
    ...
    def __init__(self, interval: int = 30, max_frames: int = 50, 
        ...
        Args:
            interval: 帧提取间隔（秒），默认30秒
            max_frames: 最大提取帧数，默认50
            enable_scene_detection: 是否启用场景检测，默认True
        """
    ...
    def _calculate_frame_timestamps(self, duration: float, 
        ...
        if scene_times:
            for scene_time in scene_times:
                # 检查是否与已有时间戳太接近（5秒内）
                is_duplicate = any(
                    abs(t[0] - scene_time) < 5 for t in timestamps
                )
    ...
    def extract(self, video_path: str, output_dir: str,
        ...
        logger.info(f"计划提取 {total_frames} 帧")
